import { DadosCliente, ItemPedido, Prato } from './types.js';

const WHATSAPP_NUMERO = process.env.WHATSAPP_NUMERO || '5521987004814';
const NOME_RESTAURANTE = process.env.NOME_RESTAURANTE || 'Sabor & Brasa';

export function formatPreco(v: number): string {
  return new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(v);
}

export function buildMensagem(itens: (ItemPedido & { prato: Prato })[], total: number, cliente: DadosCliente): string {
